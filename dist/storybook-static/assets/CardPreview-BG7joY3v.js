import{j as e}from"./jsx-runtime-BnIj46N_.js";import{r as x}from"./index-CsdIBAqE.js";import{u as n}from"./styled-components.browser.esm-BniCmjBm.js";import{c as d}from"./card-app-D2z3bZa1.js";const m=""+new URL("visaCard-CcJSL0oQ.png",import.meta.url).href,g=""+new URL("masterCard-BD-qZySX.png",import.meta.url).href,f="data:image/png;base64,",i=({text:r,type:t})=>{const a=t==="longText"?"170px":"40px";return e.jsxs(u,{width:a,children:[t==="longText"&&r,t==="text"&&r,t==="password"&&Array.from({length:r.length},()=>e.jsx(h,{src:f}))]})},u=n.span`
  height: 20px;
  width: ${({width:r})=>r};

  display: flex;
  justify-content: flex-start;
  align-items: center;

  line-height: 20px;
  font-size: 14px;
  font-weight: 500;

  letter-spacing: 2px;
`,h=n.img`
  height: 4px;
  width: 4px;
  margin-right: 5px;
`;i.__docgenInfo={description:"",methods:[],displayName:"CardText",props:{text:{required:!0,tsType:{name:"string"},description:""},type:{required:!0,tsType:{name:"union",raw:'"longText" | "text" | "password"',elements:[{name:"literal",value:'"longText"'},{name:"literal",value:'"text"'},{name:"literal",value:'"password"'}]},description:""}}};const l=({cardNumbers:r})=>{const[t,a,A,o]=r;return e.jsxs(y,{children:[e.jsx(i,{text:t,type:"text"}),e.jsx(i,{text:a,type:"text"}),e.jsx(i,{text:A,type:"password"}),e.jsx(i,{text:o,type:"password"})]})},y=n.div`
  width: 170px;
  display: flex;
  justify-content: space-between;
`;l.__docgenInfo={description:"",methods:[],displayName:"CardNumberDisplay",props:{cardNumbers:{required:!0,tsType:{name:"Array",elements:[{name:"string"}],raw:"string[]"},description:""}}};const j="/",c=({expirationDate:r})=>{const[t,a]=r,A=(o,s)=>s.length===0?o:`${o+j+s}`;return e.jsx(i,{type:"text",text:A(t,a)})};c.__docgenInfo={description:"",methods:[],displayName:"ExpirationDateDisplay",props:{expirationDate:{required:!0,tsType:{name:"Array",elements:[{name:"string"}],raw:"string[]"},description:""}}};const B={visa:m,master:g},w=()=>{const[r,t]=x.useState(null),a=s=>parseInt(s[0],10)===d.visaStartNumber,A=s=>{if(s.length<2)return!1;const p=parseInt(s.slice(0,2),10);return p>=d.minMasterNumber&&p<=d.maxMasterNumber},o=s=>{if(a(s)){t("visa");return}if(A(s)){t("master");return}t(null)};return{logoPath:r?B[r]:null,handleCardLogo:o}},I=({cardInfo:r})=>{const{cardNumbers:t,expirationDate:a,cardOwner:A}=r,{logoPath:o,handleCardLogo:s}=w();return x.useEffect(()=>{s(t[0])},[t,s]),e.jsxs(v,{children:[e.jsxs(C,{children:[e.jsx(J,{}),o&&e.jsx(P,{src:o})]}),e.jsxs(k,{children:[e.jsx(l,{cardNumbers:t}),e.jsx(c,{expirationDate:a}),e.jsx(i,{type:"longText",text:A})]})]})},v=n.div`
  height: 132px;
  width: 212px;

  color: #ffffff;
  border-radius: 4px;
  background-color: #333333;
  box-shadow: 3px 3px 5px 0px #00000040;

  margin: 0 auto;
  margin-top: 60px;
  margin-bottom: 60px;
`,C=n.div`
  display: flex;
  justify-content: space-between;
  padding: 8px 12px;
`,J=n.div`
  height: 22px;
  width: 36px;

  border-radius: 4px;
  background-color: #ddcd78;
`,P=n.img`
  height: 22px;
  width: 36px;
`,k=n.div`
  display: flex;
  flex-direction: column;

  padding-left: 17px;

  row-gap: 8px;
`;I.__docgenInfo={description:"",methods:[],displayName:"CardPreview",props:{cardInfo:{required:!0,tsType:{name:"CardInfo"},description:""}}};export{I as C};
